...
{
  # select top 1000 cpg site by Variance
  tumor.data = na.omit(as.matrix(tumor.data))
  vars = rowVars(tumor.data)
  names(vars) = rownames(tumor.data)
  dmp = names(sort(vars,decreasing=TRUE)[1:1000])
  tumor.data = tumor.data[dmp,]
  purity = as.vector(purity[colnames(tumor.data)])
  ...
  while (diff>tol & iter<maxiter) {
    ...
    for(i in 1:N_cpg){
      ...
      mu.n.new[i] = tmp1/tmp2
    }
    ## end of updating mu.n
    
    ## start updating mu
    mu.new = matrix(0,nrow = N_cpg, ncol = K)
    for(i in 1:N_cpg){
      for(k in 1:ncol(Z)){
    ...
    ## 3, update sd
    sd.new = matrix(0,nrow = N_cpg, ncol = K)
    for(i in 1:N_cpg){
      for(k in 1:K){
        func_var <- function(x){
